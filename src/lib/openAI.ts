...
export const generateQuestions = async (text: string) => {
  ...
  const questionsResponse = await client.responses.create({
    model: "gpt-5-mini",
    instructions:
      "Write 5 questions based on the given context in JSON format: { question: string, choices: Array<string>, answer: number }. There should always be 4 non-duplicates choices for each question. The answer is the index of the correct answer in the array.",
    input: notes,
  });
